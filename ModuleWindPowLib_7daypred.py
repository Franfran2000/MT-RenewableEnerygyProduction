...
def get_weather_data_Open_Meteo(lat=50.6683, long=4.6144, timez="Europe/Berlin"):
    ...
    weather["roughness_length"] = None # None because it will be using hellman wind speed model
                                                # looking at source code for how it works, if we want to use dfault value of 1/7,
...
def calculate_power_output(weather, my_turbine, e126, my_turbine2):
    ...
    mc_my_turbine = ModelChain(my_turbine, wind_speed_model="hellman", hellman_exp=1/7).run_model(weather)
    # write power output time series to WindTurbine object
    ...
    mc_example_turbine = ModelChain(
        my_turbine2, wind_speed_model="hellman", hellman_exp=1/7
    ).run_model(weather)
    my_turbine2.power_output = mc_example_turbine.power_output
    
    #Hellman good model when you don't have roughness_length parameter
